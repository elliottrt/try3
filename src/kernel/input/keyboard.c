...
static void keyboard_handler(struct Registers *regs) {
    ...
    // TODO: handlers should be as short as possible, try to
    // put keys into a buffer to handle later
    ...
    char keychar = KEY_CHAR(scancode);

    keyboard.keys[(u8) (scancode & 0x7F)] = KEY_IS_PRESS(scancode);
    keyboard.chars[keychar] = KEY_IS_PRESS(scancode);

    if (KEY_IS_PRESS(scancode) && keychar != 0) {
        cs_putchar(keychar);
    }
}
